...
rm /tmp/pkiapikey.env || true
echo "export CONJUR_AUTHN_API_KEY=${CONJUR_AUTHN_API_KEY}" > /tmp/tmp.env
echo "export CONJUR_PKI_ADMIN_API_KEY=${api_key}" >> /tmp/tmp.env
mv /tmp/tmp.env /tmp/pkiapikey.env