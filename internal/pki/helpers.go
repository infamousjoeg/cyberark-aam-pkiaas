...
// CreateCRL Generates a new CRL from the list of currently revoked certificates
func CreateCRL(revokedCertificates []types.RevokedCertificate, backend backend.Storage) httperror.HTTPError {

	revokedCertList := []pkix.RevokedCertificate{}
	expiredList, err := backend.ListExpiredCertificates(0)
	if err != nil {
		return httperror.StorageReadFail(err.Error())
	}
	for _, revokedCertificate := range revokedCertificates {
		expired := false
		for _, expiredCert := range expiredList {
			if expiredCert.String() == revokedCertificate.SerialNumber {
				expired = true
				break
			}
		}
		if !expired {
			...
			if revokedCertificate.ReasonCode > 0 {
				reasonExtension := pkix.Extension{
					...
					Value: []byte(strconv.Itoa(revokedCertificate.ReasonCode)),
				}
			...
			if !success {
				return httperror.SerialNumberConversionError("Unable to convert serial number to integer value")
			}
